...
namespace Paayes\HttpClient;

/**
 * @internal
 * @covers \Paayes\HttpClient\CurlClient
 */
final class CurlClientTest extends \PHPUnit\Framework\TestCase
{
    use \Paayes\TestHelper;
    use \Paayes\TestServer;
    ...
    {
        $this->origMaxNetworkRetries = \Paayes\Paayes::getMaxNetworkRetries();
        $this->origMaxNetworkRetryDelay = \Paayes\Paayes::getMaxNetworkRetryDelay();
        $this->origInitialNetworkRetryDelay = \Paayes\Paayes::getInitialNetworkRetryDelay();
    }
    ...
    {
        $PaayesReflector = new \ReflectionClass('\Paayes\Paayes');

        $this->maxNetworkRetryDelayProperty = $PaayesReflector->getProperty('maxNetworkRetryDelay');
        $this->maxNetworkRetryDelayProperty->setAccessible(true);

        $this->initialNetworkRetryDelayProperty = $PaayesReflector->getProperty('initialNetworkRetryDelay');
        $this->initialNetworkRetryDelayProperty->setAccessible(true);

        $curlClientReflector = new \ReflectionClass('\Paayes\HttpClient\CurlClient');
    ...
    {
        \Paayes\Paayes::setMaxNetworkRetries($this->origMaxNetworkRetries);
    ...
    {
        $fakeRandomGenerator = $this->createMock('\Paayes\Util\RandomGenerator');
    ...
    public function testDefaultOptions()
    {
        ...
        $this->expectException('Paayes\Exception\UnexpectedValueException');
    ...
    public function testShouldRetryOnPaayesShouldRetryTrue()
    {
        \Paayes\Paayes::setMaxNetworkRetries(2);
        ...
        static::assertTrue($this->shouldRetryMethod->invoke($curlClient, 0, 400, ['Paayes-should-retry' => 'true'], 0));
    }

    public function testShouldNotRetryOnPaayesShouldRetryFalse()
    {
        \Paayes\Paayes::setMaxNetworkRetries(2);
        ...
        static::assertTrue($this->shouldRetryMethod->invoke($curlClient, 0, 500, [], 0));
        static::assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 500, ['Paayes-should-retry' => 'false'], 0));
    }
    ...
    public function testShouldNotRetryAtMaximumCount()
    {
        ...
        static::assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 0, [], \Paayes\Paayes::getMaxNetworkRetries()));
    }
    ...
    public function testSleepTimeShouldGrowExponentially()
    {
        ...
        static::assertSame(
            \Paayes\Paayes::getInitialNetworkRetryDelay() * 1,
            $this->sleepTimeMethod->invoke($curlClient, 1, [])
        );
        static::assertSame(
            \Paayes\Paayes::getInitialNetworkRetryDelay() * 2,
            $this->sleepTimeMethod->invoke($curlClient, 2, [])
        );
        static::assertSame(
            \Paayes\Paayes::getInitialNetworkRetryDelay() * 4,
            $this->sleepTimeMethod->invoke($curlClient, 3, [])
        );
        static::assertSame(
            \Paayes\Paayes::getInitialNetworkRetryDelay() * 8,
    ...
    public function testSleepTimeShouldAddSomeRandomness()
    {
        ...
        $baseValue = \Paayes\Paayes::getInitialNetworkRetryDelay() * (0.5 * (1 + $randomValue));

        // the initial value cannot be smaller than the base,
        // so the randomness is ignored
        static::assertSame(\Paayes\Paayes::getInitialNetworkRetryDelay(), $this->sleepTimeMethod->invoke($curlClient, 1, []));
    ...
    public function testSetRequestStatusCallback()
    {
        try {
            ...
            \Paayes\ApiRequestor::setHttpClient(null);
        }
    }
...
header("Paayes-should-retry", "true");
?>
{}
EOF;

        \Paayes\Paayes::setMaxNetworkRetries(3);
        $this->fastRetries();
...
EOF;
        ...
        static::assertSame('Paayes\Exception\ApiConnectionException', \get_class($exception));
    ...
    {
        $curl = new CurlClient();
        $coupon = \Paayes\Coupon::retrieve('coupon_xyz');

        $absUrl = \Paayes\Paayes::$apiBase . '/api/v1/coupons/xyz';
        $opts[\CURLOPT_HTTPGET] = 1;
        $opts[\CURLOPT_URL] = $absUrl;
        $opts[\CURLOPT_HTTPHEADER] = ['Authorization: Basic c2tfdGVzdF94eXo6'];
