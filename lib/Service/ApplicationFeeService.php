...
class ApplicationFeeService extends \Paayes\Service\AbstractService
{
    /**
    ...
    {
        return $this->requestCollection('get', '/api/v1/application_fees', $params, $opts);
    }
    ...
    {
        return $this->requestCollection('get', $this->buildPath('/api/v1/application_fees/%s/refunds', $parentId), $params, $opts);
    }

    /**
     * Refunds an application fee that has previously been collected but not yet
     * refunded. Funds will be refunded to the Paayes account from which the fee was
    ...
    {
        return $this->request('post', $this->buildPath('/api/v1/application_fees/%s/refunds', $parentId), $params, $opts);
    }
     ...
     * @throws \Paayes\Exception\ApiErrorException if the request fails
     *
     * @return \Paayes\ApplicationFee
     */
    public function retrieve($id, $params = null, $opts = null)
    {
        return $this->request('get', $this->buildPath('/api/v1/application_fees/%s', $id), $params, $opts);
    }
    ...
    {
        return $this->request('get', $this->buildPath('/api/v1/application_fees/%s/refunds/%s', $parentId, $id), $params, $opts);
    }
    ...
    {
        return $this->request('post', $this->buildPath('/api/v1/application_fees/%s/refunds/%s', $parentId, $id), $params, $opts);
    }
}
