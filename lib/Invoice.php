 ...
 *
 * They contain <a href="https://docs.paayes.com/docs/api#invoiceitems">invoice
 * items</a>, and proration adjustments that may be caused by subscription
 * upgrades/downgrades (if necessary).
 *
 * If your invoice is configured to be billed through automatic charges, Paayes
 * automatically finalizes your invoice and attempts payment. Note that finalizing
 * the invoice, <a
 * href="https://docs.paayes.com/docs/billing/invoices/workflow/#auto_advance">when
 * automatic</a>, does not happen immediately as the invoice is created. Paayes
 * waits until one hour after the last webhook was successfully sent (or the last
 * webhook timed out after failing). If you (and the platforms you may have
 * connected to) have no webhooks configured, Paayes waits one hour after creation
 * to finalize the invoice.
 *
 * If your invoice is configured to be billed by sending an email, then based on
 * your <a href="https://dashboard.paayes.com/account/billing/automatic">email
 * settings</a>, Paayes will email the invoice to your customer and await payment.
 * These emails can contain a link to a hosted page to pay the invoice.
 *
 * Paayes applies any customer credit on the account before determining the amount
 * due for the invoice (i.e., the amount that will be actually charged). If the
 * amount due for the invoice is less than Paayes's <a
 ...
 * href="https://docs.paayes.com/docs/billing/customer/balance">here</a>.
 *
 * Related guide: <a href="https://docs.paayes.com/docs/billing/invoices/sending">Send
 ...
 * @property null|(string|\Paayes\TaxId)[] $account_tax_ids The account tax IDs associated with the invoice. Only editable when the invoice is a draft.
 ...
 * @property null|int $application_fee_amount The fee in %s that will be applied to the invoice and transferred to the application owner's Paayes account when the invoice is paid.
 ...
 * @property bool $auto_advance Controls whether Paayes will perform <a href="https://docs.paayes.com/docs/billing/invoices/workflow/#auto_advance">automatic collection</a> of the invoice. When <code>false</code>, the invoice's state will not automatically advance without an explicit action.
 * @property \Paayes\PaayesObject $automatic_tax
 ...
 * @property null|string|\Paayes\Charge $charge ID of the latest charge generated for this invoice, if any.
 * @property string $collection_method Either <code>charge_automatically</code>, or <code>send_invoice</code>. When charging automatically, Paayes will attempt to pay this invoice using the default source attached to the customer. When sending an invoice, Paayes will email this invoice to the customer with payment instructions.
 ...
 * @property null|\Paayes\PaayesObject[] $custom_fields Custom fields displayed on the invoice.
 * @property null|string|\Paayes\Customer $customer The ID of the customer who will be billed.
 * @property null|\Paayes\PaayesObject $customer_address The customer's address. Until the invoice is finalized, this field will equal <code>customer.address</code>. Once the invoice is finalized, this field will no longer be updated.
 ...
 * @property null|\Paayes\PaayesObject $customer_shipping The customer's shipping information. Until the invoice is finalized, this field will equal <code>customer.shipping</code>. Once the invoice is finalized, this field will no longer be updated.
 ...
 * @property null|\Paayes\PaayesObject[] $customer_tax_ids The customer's tax IDs. Until the invoice is finalized, this field will contain the same tax IDs as <code>customer.tax_ids</code>. Once the invoice is finalized, this field will no longer be updated.
 * @property null|string|\Paayes\PaymentMethod $default_payment_method ID of the default payment method for the invoice. It must belong to the customer associated with the invoice. If not set, defaults to the subscription's default payment method, if any, or to the default payment method in the customer's invoice settings.
 * @property null|string|\Paayes\Account|\Paayes\AlipayAccount|\Paayes\BankAccount|\Paayes\BitcoinReceiver|\Paayes\Card|\Paayes\Source $default_source ID of the default payment source for the invoice. It must belong to the customer associated with the invoice and be in a chargeable state. If not set, defaults to the subscription's default source, if any, or to the customer's default source.
 * @property \Paayes\TaxRate[] $default_tax_rates The tax rates applied to this invoice, if any.
 ...
 * @property null|\Paayes\Discount $discount Describes the current discount applied to this invoice, if there is one. Not populated if there are multiple discounts.
 * @property null|(string|\Paayes\Discount)[] $discounts The discounts applied to the invoice. Line item discounts are applied before invoice discounts. Use <code>expand[]=discounts</code> to expand each discount.
 ...
 * @property null|\Paayes\ErrorObject $last_finalization_error The error encountered during the previous attempt to finalize the invoice. This field is cleared when the invoice is successfully finalized.
 * @property \Paayes\Collection $lines The individual line items that make up the invoice. <code>lines</code> is sorted as follows: invoice items in reverse chronological order, followed by the subscription, if any.
 ...
 * @property null|string|\Paayes\PaymentIntent $payment_intent The PaymentIntent associated with this invoice. The PaymentIntent is generated when the invoice is finalized, and can then be used to pay the invoice. Note that voiding an invoice will cancel the PaymentIntent.
 * @property \Paayes\PaayesObject $payment_settings
 ...
 * @property null|string|\Paayes\Quote $quote The quote this invoice was generated from.
 ...
 * @property null|string $status The status of the invoice, one of <code>draft</code>, <code>open</code>, <code>paid</code>, <code>uncollectible</code>, or <code>void</code>. <a href="https://docs.paayes.com/docs/billing/invoices/workflow#workflow-overview">Learn more</a>
 * @property \Paayes\PaayesObject $status_transitions
 * @property null|string|\Paayes\Subscription $subscription The subscription that this invoice was prepared for, if any.
 ...
 * @property \Paayes\PaayesObject $threshold_reason
 * @property int $total Total after discounts and taxes.
 * @property null|\Paayes\PaayesObject[] $total_discount_amounts The aggregate amounts calculated per discount across all line items.
 * @property \Paayes\PaayesObject[] $total_tax_amounts The aggregate amounts calculated per tax rate for all line items.
...
class Invoice extends ApiResource
{
     ...
     *
     * @throws PaayesExceptionApiErrorException if the request fails
     *
     * @return \Paayes\Collection the list of lines (InvoiceLineItem)
     */
     ...
     *
     * @return \Paayes\Invoice the finalized invoice
     */
     ...
     * @throws \Paayes\Exception\ApiErrorException if the request fails
     *
     * @return \Paayes\Invoice the uncollectible invoice
     */
     ...
     * @throws \Paayes\Exception\ApiErrorException if the request fails
     *
     * @return \Paayes\Invoice the paid invoice
     */
     ...
     * @throws \Paayes\Exception\ApiErrorException if the request fails
     *
     * @return \Paayes\Invoice the sent invoice
     */
     ...
     * @throws \Paayes\Exception\ApiErrorException if the request fails
     *
     * @return \Paayes\Invoice the upcoming invoice
     */
     ...
     * @throws \Paayes\Exception\ApiErrorException if the request fails
     *
     * @return \Paayes\Invoice the voided invoice
     */
