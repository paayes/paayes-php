 ...
 *
 * Related guide: <a
 * href="https://docs.paayes.com/docs/billing/subscriptions/creating">Creating
 * Subscriptions</a>.
 *
 * @property string $id Unique identifier for the object.
 * @property string $object String representing the object's type. Objects of the same type share the same value.
 * @property null|float $application_fee_percent A non-negative decimal between 0 and 100, with at most two decimal places. This represents the percentage of the subscription invoice subtotal that will be transferred to the application owner's Paayes account.
 ...
 * @property null|\Paayes\PaayesObject $billing_thresholds Define thresholds at which an invoice will be sent, and the subscription advanced to a new billing period
 ...
 * @property string $collection_method Either <code>charge_automatically</code>, or <code>send_invoice</code>. When charging automatically, Paayes will attempt to pay this subscription at the end of the cycle using the default source attached to the customer. When sending an invoice, Paayes will email your customer an invoice with payment instructions.
 ...
 * @property string|\Paayes\Customer $customer ID of the customer who owns the subscription.
 ...
 * @property null|string|\Paayes\Account|\Paayes\AlipayAccount|\Paayes\BankAccount|\Paayes\BitcoinReceiver|\Paayes\Card|\Paayes\Source $default_source ID of the default payment source for the subscription. It must belong to the customer associated with the subscription and be in a chargeable state. If <code>default_payment_method</code> is also set, <code>default_payment_method</code> will take precedence. If neither are set, invoices will use the customer's <a href="https://docs.paayes.com/api/customers/object#customer_object-invoice_settings-default_payment_method">invoice_settings.default_payment_method</a> or <a href="https://docs.paayes.com/api/customers/object#customer_object-default_source">default_source</a>.
 * @property null|\Paayes\TaxRate[] $default_tax_rates The tax rates that will apply to any subscription item that does not have <code>tax_rates</code> set. Invoices created will have their <code>default_tax_rates</code> populated from the subscription.
 * @property null|\Paayes\Discount $discount Describes the current discount applied to this subscription, if there is one. When billing, a discount applied to a subscription overrides a discount applied on a customer-wide basis.
 * @property null|int $ended_at If the subscription has ended, the date the subscription ended.
 * @property \Paayes\Collection $items List of subscription items, each with an attached price.
 * @property null|string|\Paayes\Invoice $latest_invoice The most recent invoice this subscription has generated.
 ...
 * @property null|\Paayes\PaayesObject $pause_collection If specified, payment collection for this subscription will be paused.
 * @property null|\Paayes\PaayesObject $payment_settings Payment settings passed on to invoices created by the subscription.
 * @property null|\Paayes\PaayesObject $pending_invoice_item_interval Specifies an interval for how often to bill for any pending invoice items. It is analogous to calling <a href="https://docs.paayes.com/docs/api#create_invoice">Create an invoice</a> for the given subscription at the specified interval.
 * @property null|string|\Paayes\SetupIntent $pending_setup_intent You can use this <a href="https://docs.paayes.com/api/setup_intents">SetupIntent</a> to collect user authentication when creating a subscription without immediate payment or updating a subscription's payment method, allowing you to optimize for off-session payments. Learn more in the <a href="https://docs.paayes.com/docs/billing/migration/strong-customer-authentication#scenario-2">SCA Migration Guide</a>.
 * @property null|\Paayes\PaayesObject $pending_update If specified, <a href="https://docs.paayes.com/docs/billing/subscriptions/pending-updates">pending updates</a> that will be applied to the subscription once the <code>latest_invoice</code> has been paid.
 * @property null|string|\Paayes\SubscriptionSchedule $schedule The schedule attached to the subscription
...
class Subscription extends ApiResource
{
     ...
     *
     * @return \Paayes\Subscription the deleted subscription
     */
     ...
     * @throws \Paayes\Exception\ApiErrorException if the request fails
     *
     * @return \Paayes\Subscription the updated subscription
     */
