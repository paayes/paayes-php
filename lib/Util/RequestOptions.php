    ...
    public static function parse($options, $strict = false)
    {
        ...
        if (\is_string($options)) {
            if ($strict) {
                ...
                throw new \Paayes\Exception\InvalidArgumentException($message);
            }
        ...
        if (\is_array($options)) {
            ...
            if (\array_key_exists('Paayes_account', $options)) {
                $headers['Paayes-Account'] = $options['Paayes_account'];
                unset($options['Paayes_account']);
            }
            if (\array_key_exists('Paayes_version', $options)) {
                $headers['Paayes-Version'] = $options['Paayes_version'];
                unset($options['Paayes_version']);
            }
        ...
        $message = 'The second argument to Paayes API method calls is an '
            . 'optional per-request apiKey, which must be a string, or '
            . 'per-request options, which must be an array. (HINT: you can set '
            . 'a global apiKey by "Paayes::setApiKey(<apiKey>)")';
