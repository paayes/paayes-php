...
abstract class Util
{
    ...
    /**
     * Converts a response from the Paayes API to the corresponding PHP object.
     *
     * @param array $resp the response from the Paayes API
     * @param array $opts
     *
     * @return array|PaayesObject
     */
    public static function convertToPaayesObject($resp, $opts)
    {
        $types = \Paayes\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
                $mapped[] = self::convertToPaayesObject($i, $opts);
            }
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \Paayes\PaayesObject::class;
            }
        ...
        if (null === self::$isMbstringAvailable) {
            ...
            if (!self::$isMbstringAvailable) {
                \trigger_error('It looks like the mbstring extension is not enabled. ' .
                    ...
                    'support@paayes.com if you have any questions.', \E_USER_WARNING);
            }
        }
    ...
    {
        if ($h instanceof \Paayes\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
