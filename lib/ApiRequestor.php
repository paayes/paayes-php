...
class ApiRequestor
{
    ...
    private static $OPTIONS_KEYS = ['api_key', 'idempotency_key', 'Paayes_account', 'Paayes_version', 'api_base'];
    ...
    public function __construct($apiKey = null, $apiBase = null)
    {
        $this->_apiKey = $apiKey;
        if (!$apiBase) {
            $apiBase = Paayes::$apiBase;
        }
        $this->_apiBase = $apiBase;
    }

    /**
     * Creates a telemetry json blob for use in 'X-Paayes-Client-Telemetry' headers.
     *
     * @static
     *
    ...
    private static function _telemetryJson($requestTelemetry)
    {
        ...
        Paayes::getLogger()->error('Serializing telemetry payload failed!');
    ...
    {
        $uaString = 'Paayes/v1 PhpBindings/' . Paayes::VERSION;

        $langVersion = \PHP_VERSION;
        $uname_disabled = static::_isDisabled(\ini_get('disable_functions'), 'php_uname');
        $uname = $uname_disabled ? '(disabled)' : \php_uname();

        $appInfo = Paayes::getAppInfo();
        $ua = [
            'bindings_version' => Paayes::VERSION,
            'lang' => 'php',
        ...
        return [
            'X-Paayes-Client-User-Agent' => \json_encode($ua),
            'User-Agent' => $uaString,
    ...
    private function _prepareRequest($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = Paayes::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
                . '"Paayes::setApiKey(<API-KEY>)".  You can generate API keys from '
                . 'the Paayes web interface.  See https://paayes.com/api for '
                . 'details, or email support@paayes.com if you have any questions.';

            throw new Exception\AuthenticationException($msg);
        }

        // Clients can supply arbitrary additional keys to be included in the
        // X-Paayes-Client-User-Agent header via the optional getUserAgentInfo()
        // method
        $clientUAInfo = null;
        ...
        if (Paayes::$apiVersion) {
            $defaultHeaders['Paayes-Version'] = Paayes::$apiVersion;
        }

        if (Paayes::$accountId) {
            $defaultHeaders['Paayes-Account'] = Paayes::$accountId;
        }

        if (Paayes::$enableTelemetry && null !== self::$requestTelemetry) {
            $defaultHeaders['X-Paayes-Client-Telemetry'] = self::_telemetryJson(self::$requestTelemetry);
        }
