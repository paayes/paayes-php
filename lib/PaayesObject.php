 ...
 * Class PaayesObject.
 */
class PaayesObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToPaayesObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_transientValues) && $this->_transientValues->includes($k)) {
            ...
            $message = "Paayes Notice: Undefined property of {$class} instance: {$k}. "
                . "HINT: The {$k} attribute was set in the past, however. "
                . 'It was then wiped when refreshing the object '
                . "with the result returned by Paayes's API, "
                . 'probably as a result of a save(). The attributes currently '
                . "available on this object are: {$attrs}";
            Paayes::getLogger()->error($message);

            return $nullval;
        }
        $class = static::class;
        Paayes::getLogger()->error("Paayes Notice: Undefined property of {$class} instance: {$k}");
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof PaayesObject) {
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a PaayesObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if (('metadata' === $k) && (\is_array($v))) {
                $this->_values[$k] = PaayesObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToPaayesObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a PaayesObject. A PaayesObject may contain modified
            //      values within in that its parent PaayesObject doesn't know about.
            //
            $original = \array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof PaayesObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToPaayesObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof PaayesObject) {
    ...
    /**
     * Returns a pretty JSON representation of the Paayes object.
     *
     * @return string the JSON representation of the Paayes object
     */
    ...
    /**
     * Sets all keys within the PaayesObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any PaayesObjects contained as values or
    ...
    protected static function deepCopy($obj)
    {
        ...
        if ($obj instanceof PaayesObject) {
    ...
    public static function emptyValues($obj)
    {
        if (\is_array($obj)) {
            $values = $obj;
        } elseif ($obj instanceof PaayesObject) {
            $values = $obj->_values;
        } else {
            throw new Exception\InvalidArgumentException(
    ...
    /**
     * @return null|ApiResponse The last response from the Paayes API
     */
    ...
    /**
     * Sets the last response from the Paayes API.
     *
