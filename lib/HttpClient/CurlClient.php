...
class CurlClient implements ClientInterface, StreamingClientInterface
{
    ...
    /** @var \Paayes\Util\RandomGenerator */
     ...
     * @param null|\Paayes\Util\RandomGenerator $randomGenerator
     */
     ...
     *   <li>\Paayes\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (('post' === $method || 'put'=== strtolower($method)) && (Paayes::$maxNetworkRetries > 0)) {
        ...
        $opts[\CURLOPT_CAINFO] = Paayes::getCABundlePath();
        if (!Paayes::getVerifySslCerts()) {
        ...
        // Paayes's API servers are only accessible over IPv4. Force IPv4 resolving to avoid
        // potential issues (cf. https://github.com/Paayes/Paayes-php/issues/1045).
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Paayes ({$url}).  Please check your "
                    . 'internet connection and try again.  If this problem persists, '
                    . "you should check Paayes's service status at "
                    . 'https://twitter.com/Paayesstatus, or';

                break;

            case \CURLE_SSL_CACERT:
            case \CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Paayes's SSL certificate.  Please make sure "
            ...
            default:
                $msg = 'Unexpected error communicating with Paayes.  '
                    . 'If this problem persists,';
        }
        $msg .= ' let us know at support@paayes.com.';
     ...
     * @param int $errno
     * @param int $rcode
     * @param array|\Paayes\Util\CaseInsensitiveArray $rheaders
     * @param int $numRetries
     *
     * @return bool
     */
    ...
    {
        if ($numRetries >= Paayes::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['Paayes-should-retry'])) {
            if ('false' === $rheaders['Paayes-should-retry']) {
                return false;
            }
            if ('true' === $rheaders['Paayes-should-retry']) {
                return true;
            }
        }
        ...
        //
        // Note that we expect the Paayes-should-retry header to be false
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            Paayes::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            Paayes::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(Paayes::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= Paayes::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which Paayes use.
