...
trait Request
{
     ...
     *
     * @throws \Paayes\Exception\InvalidArgumentException if $params exists and is not an array
     */
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            $message = 'You must pass an array as the first argument to Paayes API '
                . 'method calls.  (HINT: an example call to create a charge '
                . "would be: \"Paayes\\Charge::create(['amount' => 100, "
    ...
    {
        $opts = \Paayes\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \Paayes\ApiRequestor($opts->apiKey, $baseUrl);
