...
\Paayes\Paayes::setApiKey(\getenv('Paayes_SECRET_KEY'));
\Paayes\Paayes::setClientId(\getenv('Paayes_CLIENT_ID'));
...
if (isset($_GET['code'])) {
    ...
    try {
        $resp = \Paayes\OAuth::token([
            'grant_type' => 'authorization_code',
            'code' => $code,
        ]);
    } catch (\Paayes\Exception\OAuth\OAuthErrorException $e) {
        exit('Error: ' . $e->getMessage());
    }

    $accountId = $resp->Paayes_user_id;
    ...
    try {
        \Paayes\OAuth::deauthorize([
            'Paayes_user_id' => $accountId,
        ]);
    } catch (\Paayes\Exception\OAuth\OAuthErrorException $e) {
        exit('Error: ' . $e->getMessage());
    }
    ...
    $url = \Paayes\OAuth::authorizeUrl([
        'scope' => 'read_only',
    ]);
    echo "<a href=\"{$url}\">Connect with Paayes</a>\n";
}
